...
#   
#   Once it runs through the specific post/site, it will then print out details of the bots
#    while doing a well documented report of them for the user.
# 
...
class WebBotWindow:
    ...
    def __init__(self):
        ...
        self.msg_Queue= queue.Queue() # For staying on sync with the agents and terminal, the queue class is used. This will also be used by the main.py file for consistency and simplicity sake for messages.
        ...
        # 3/18/2026 from the first widget line here to the last widget line in this constructor, this was from the construct widgets.
        ...
        self.text_output_Box.see(tk.END) # --Lets user scroll in the text box.
        self.text_output_Box.config(state=tk.DISABLED) # --Sets the text to read-only (since this is intented for read-only, not read and write.).
        self.text_output_Box.pack() # --Packs the Message Box. (This is the first widget to pack, due to it being intented to be at the top portion of the window.)
        ...
        # -- This constructs the submit button, where this takes in the the flow section that kick off the crews (specifically input_Operation()), and uses that as a command.
        self.submit_button = tk.Button(self.root, text= "Submit", command= self.override_button_command(return_agents_Thread= None)) # With the button, the submit button is acutally set to None first.
...
# --Returns the entry's values to the agents in str format.
    ...
    def submit_input(self, agents):
        ...
        # --If the website URL, web type, and bot type are not empty, then submit inputs and return the agents to start the agents background thread of the flow.
        if(self.website_URL != "" and self.web_type != "" and self.bot_type != ""):
            ...
            self.msg_Queue.put(f"System: Agents are now currently looking at {data['website_url']}")
            self.message_Stream(self.msg_Queue.get_nowait())

            self.agents = agents # gets the agents thread.
            agents # uses the agents thread.
        else:
            self.msg_Queue.put(f"Invalid Inputs! Inputs must be Strings and URL must not be invalid!")
            self.message_Stream(self.msg_Queue.get_nowait()) # --Otherwise, message prints out that the fields are empty.
            self.unlock_Submit_Input # --Points to the unlock_Submit_Input method, where the user tries again to enter appropriate inputs.

        return self.agents # --returns the agents for the agents background thread. This will be the output for clicking the submit button.

    # --Submit input function ends here.

    # --Override button command function starts here.
    def override_button_command(self, return_agents_Thread = None):
        """Overrides the button command button."""
        self.return_agents_Thread = return_agents_Thread
        self.return_agents_Thread
    # --Override button command function ends here.
    ...
    def refreshWindow(self, new_message: str): # new_message must be a string
        """This checks the queues from messages by the agents every 100ms. (This just refreshes the textbox.)"""
        try:
            while(True):
                self.new_message = new_message 
                self.new_message = self.msg_Queue.put() # First, gets the message and puts it in a queue.
                self.message_Stream(self.msg_Queue.get_nowait()) # Then, retreives the message, if non-null.
        except Exception:
            pass
        self.root.after(100, self.refreshWindow) # Waits 100 milliseconds, and call itself to check a new message, and so on.
